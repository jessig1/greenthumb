...
class PlantedPlantControllerTest {
    ...
    void createThenListReturnsPlantingForContainerOwner() throws Exception {
        String containerId = createContainer("alice");
        ...
        mockMvc.perform(get("/api/v1/containers/" + containerId + "/plantings").header("X-Dev-User-Id", "alice"))
    ...
    void otherUserCannotAccessPlantingInSomeoneElsesContainer() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/containers/" + containerId + "/plantings").header("X-Dev-User-Id", "bob"))
                .andExpect(status().isNotFound());

        mockMvc.perform(get("/api/v1/plantings/" + plantingId).header("X-Dev-User-Id", "bob"))
                .andExpect(status().isNotFound());

        mockMvc.perform(delete("/api/v1/plantings/" + plantingId).header("X-Dev-User-Id", "bob"))
                .andExpect(status().isNotFound());
    }
    ...
    private String createContainer(String user) throws Exception {
